...
class PLSPlotMixin(ABCMeta):
    ...
    def plotScores(self, comps=[0, 1], col=None):

        if len(comps) == 1:
            plt.scatter(range(self.scores_t.shape[0]))
        else:
            plt.scatter(self..scores[:, 0], self.scores_t[:, 1])
    ...
    def plotPermutationResults(self, metric='Q2'):
