...
class ChemometricsProbabilisticQuotientNormaliser(BaseEstimator, TransformerMixin):
    """
    A scikit-learn like scaler object
    ...
    def __init__(self, copy=True):
        self.copy = copy
        self.normalisation_coefs_ = None
    ...
    def fit(self, X, reference=None):
        ...
        try:
            if reference == None:
                self.reference_ = numpy.nanmedian(X, axis=0)

            fold_change_matrix = X/self.reference_

            self.normalisation_coefs_ = numpy.median(fold_change_matrix, axis=1)
    ...
    def fit_transform(self, X, copy=True):
        ...
        X = X/self.normalisation_coefs[:, None]
    ...
    def _inverse_transform(self, X, copy=None):
